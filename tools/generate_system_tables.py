#!/usr/bin/env python3
"""Generate the XP-60 System parameter tables from the protocol document.

Source of truth: docs/protocol/XP60_SYSTEM_PARAMETER_MAP.md (transcribed from
the Roland XP-60/XP-80 MIDI Implementation, Parameter Address Map §1-1). This
script turns its System Common and Scale Tune tables into
src/xpmodel/generated/Xp60SystemTables.{h,cpp}, so every descriptor in code is
...
Usage:
    tools/generate_system_tables.py            # rewrite the generated files
    tools/generate_system_tables.py --check    # exit 1 if the files are stale
"""
...
DOC = ROOT / "docs" / "protocol" / "XP60_SYSTEM_PARAMETER_MAP.md"
OUT_DIR = ROOT / "src" / "xpmodel" / "generated"
OUT_H = OUT_DIR / "Xp60SystemTables.h"
OUT_CPP = OUT_DIR / "Xp60SystemTables.cpp"

# Sixteen Part Scale Tune blocks plus the Patch-mode one.
SCALE_BLOCKS = 17

CATEGORY_RULES = [
    ("Scale Tune for", "Scale Tune"),
    ("Sound Mode", "Mode"),
    ("Performance Number", "Mode"),
    ("Patch Group", "Mode"),
    ("Patch Number", "Mode"),
    ("Master Tune", "Tuning"),
    ("Scale Tune Switch", "Tuning"),
    ("Transpose", "Tuning"),
    ("Arpeggio", "Arpeggio"),
    ...
    ("Transmit", "MIDI"),
    ("Channel", "MIDI"),
    ("Control Source", "Controllers"),
    ("Assign", "Controllers"),
    ("Output Mode", "Controllers"),
    ("Polarity", "Controllers"),
    ("Keyboard", "Keyboard"),
    ("Aftertouch", "Keyboard"),
    ("Octave Shift", "Keyboard"),
    ("Switch", "Switches"),
    ("Clock Source", "Sync"),
]
...
def ident_for(row: Row) -> Tuple[str, str]:
    m = re.match(r"^Scale Tune for (.+)$", row.name)
    if m:
        # "C#" must not pascal down to "C": the sharps would collide with the
        # naturals and silently give two enumerators the same name.
        note = m.group(1).replace("#", " Sharp")
        return f"scale.{snake(note)}", f"ScaleTune{pascal(note)}"
...
def parse_tables(doc: str) -> Tuple[List[Row], Dict[str, int]]:
    ...
    for line in doc.splitlines():
        if line.startswith("## 1. System Common"):
            block = "common"
        elif line.startswith("## 2. Scale Tune"):
            block = "scale"
...
def check_unique_identifiers(rows: List[Row]) -> None:
    """No two rows of a block may share an identifier or an enumerator.

    A collision is silent in C++ only until it is not — two enumerators of the
    same name are a compile error, but two rows resolving to the same id would
    make every lookup ambiguous. Catching it here names the rows.
    """
    for block in {row.block for row in rows}:
        for field in ("ident", "enumerator"):
            seen: Dict[str, str] = {}
            for row in (r for r in rows if r.block == block):
                value = getattr(row, field)
                if value in seen:
                    raise SystemExit(f"{block}: '{seen[value]}' and '{row.name}' both use {field} '{value}'")
                seen[value] = row.name
...
def generate(rows: List[Row], sizes: Dict[str, int], digest: str) -> Tuple[str, str]:
    ...
    part = [r for r in rows if r.block == "scale"]
    ...
    header = [
        ...
        "",
        f"inline constexpr std::uint32_t kSystemCommonSize = {sizes['common']};",
        f"inline constexpr std::uint32_t kScaleTuneSize = {sizes['scale']};",
        "// Seventeen Scale Tune blocks: one per Performance Part at Roland",
        "// 10 00 .. 1F 00, and one for Patch mode at 20 00.",
        f"inline constexpr int kScaleTuneBlockCount = {SCALE_BLOCKS};",
        "inline constexpr std::array<std::uint32_t, {}> kScaleTuneOffsets{{{{{}}}}};".format(
            SCALE_BLOCKS, ", ".join(str(roland_offset(f"{0x10 + i:02X} 00")) for i in range(SCALE_BLOCKS))),
        "// The Patch-mode block is the last of them.",
        "inline constexpr std::uint32_t kPatchModeScaleTuneOffset = {};".format(roland_offset("20 00")),
        "// Distance from the System base to the byte after the last Scale Tune.",
        "inline constexpr std::uint32_t kSystemSpan = {};".format(roland_offset("20 00") + sizes["scale"]),
        "",
...
def main(argv: List[str]) -> int:
    ...
    if args.check:
        for path, produced in ((OUT_H, header), (OUT_CPP, source)):
            if not path.exists() or path.read_text(encoding="utf-8") != produced:
                print(f"{path.name} is stale; run tools/generate_system_tables.py", file=sys.stderr)
                return 1
        return 0
    ...
    print(f"{len([r for r in rows if r.block == 'common'])} Common + "
          f"{len([r for r in rows if r.block == 'scale'])} Scale Tune rows written")
    return 0
