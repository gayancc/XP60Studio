...
| 15 | **Wave Group ID to board** | With a known SR-JV80 board installed in a known slot, select one of its waves in a Tone from the front panel and read the Tone back. Repeat for a second board and a second slot. Establishes whether the Wave Group ID names the board, the slot, or something else — and explains group 97, which fits neither. | [`ROLAND_XP60_PROTOCOL_FACTS.md`](protocol/ROLAND_XP60_PROTOCOL_FACTS.md) §7 | Phase 7 expansion intelligence; the analysis works without it but must answer "unknown" more often | Not started |
