...
| 8 | **EFX parameter slots** | Which Patch Common byte each algorithm's front-panel parameter occupies, and its raw-to-display conversion | [`PHASE_4_EFFECT_ROUTING.md`](PHASE_4_EFFECT_ROUTING.md) § Remaining EFX slot evidence, steps 1–6 | Phase 4 M2 close | **Partly 2026-09-04**: 3 algorithms of 40 mapped (02:OVER-DRIVE, 03:DISTORTION, 04:PHASER). Settled that a control's slot is **per-algorithm**: Pan is byte 2 in DISTORTION/OVER-DRIVE and byte 6 in PHASER, so the printed table cannot be trusted for any algorithm not swept. Range coverage thin, no displayed value read, conversions untested. Remaining 37 are a session of their own. |
