...
| 8 | **EFX parameter slots** | Which Patch Common byte each algorithm's front-panel parameter occupies, and its raw-to-display conversion | [`PHASE_4_EFFECT_ROUTING.md`](PHASE_4_EFFECT_ROUTING.md) § Remaining EFX slot evidence, steps 1–6 | Phase 4 M2 close | **Partly 2026-09-04**: 03:DISTORTION fully mapped — all six controls tied to bytes 1-6, Pan displaced to byte 2 where the printed order has Amp Type. Range coverage is thin (only Drive reached both endpoints) and no displayed value was read off the instrument, so raw-to-display conversions stay untested. 1 algorithm of 40. |
