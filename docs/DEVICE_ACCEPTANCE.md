...
| 15 | **Wave Group ID to board** | With a known SR-JV80 board installed in a known slot, select one of its waves in a Tone from the front panel and read the Tone back. Repeat for a second board and a second slot. Confirms or refutes the mapping XP60Studio infers: that the Wave Group ID is the SR-JV80 catalogue number. Slot indices are ruled out (they would be 1..4); every group seen in real user data is a real board number, 97 included (Experience III), but that is consistency rather than proof. | [`ROLAND_XP60_PROTOCOL_FACTS.md`](protocol/ROLAND_XP60_PROTOCOL_FACTS.md) §7 | Phase 7 expansion intelligence; the analysis works without it — the board catalogue only names groups and pre-fills the picker, and Learn overrides it — but confirmation would let the catalogue be trusted rather than merely offered | Not started |
