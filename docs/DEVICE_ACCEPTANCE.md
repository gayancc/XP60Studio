...
| 8 | **EFX parameter slots** | Which Patch Common byte each algorithm's front-panel parameter occupies, and its raw-to-display conversion | [`PHASE_4_EFFECT_ROUTING.md`](PHASE_4_EFFECT_ROUTING.md) § Remaining EFX slot evidence, steps 1–6 | Phase 4 M2 close | **Partly 2026-09-04**: changing the algorithm rewrites the shared parameter bytes (3 observations), and single panel edits move exactly one documented byte with no coupling. No slot assignment established: the displayed parameter names were not recorded, so the mapping stays Unverified. |
